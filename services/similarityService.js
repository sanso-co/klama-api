...
export const calculateSimilarity = async (showId) => {
    const targetShow = await Show.findOne({ id: showId }).populate("genres").populate("keywords");
    ...
    const similarityScores = filteredShows.map((show) => {
        ...
        show.keywords.forEach((kw) => {
            if (kw.id < 2000) {
                const matchingKeyword = targetShow.keywords.find((tkw) => tkw._id.equals(kw._id));
                if (matchingKeyword) {
